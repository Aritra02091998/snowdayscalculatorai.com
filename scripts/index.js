...
let currentForecast = [];
let currentAddress  = "";  // ← new: store the last‐used address
...
async function calculateProbability() {
    ...
    currentAddress = addr;      // ← new: remember what the user entered
    ...
    try {
        ...
        if (e === "Location outside the US") {
            // ← clear the address input and re-enable state/ZIP
...
function generatePDF() {
    ...
    // — User-entered address (larger, bold, green like "School expected OPEN") —
